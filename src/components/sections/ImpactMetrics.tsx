      ...
      <Container>
        <Reveal>
          <p className="mb-10 text-center text-sm font-medium uppercase tracking-[0.2em] text-faint">
            Measurable impact, backed by production work
          </p>
        </Reveal>
        <dl className="grid grid-cols-2 gap-4 sm:gap-5 lg:grid-cols-4">
          {metrics.map((metric, i) => (
            <Reveal key={metric.label} delay={i * 90}>
              <div className="ring-glow group relative h-full overflow-hidden rounded-3xl glass p-5 sm:p-7">
                <dt className="sr-only">{metric.label}</dt>
                <dd>
                  <span className="block whitespace-nowrap bg-gradient-to-r from-cyan-300 to-indigo-300 bg-clip-text text-2xl font-semibold tracking-tight text-transparent sm:text-3xl md:text-4xl lg:text-5xl">
